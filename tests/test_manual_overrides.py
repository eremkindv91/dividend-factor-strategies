...
def test_manual_override_ignores_review_template_rows_until_marked_ready(tmp_path: Path):
    processed = tmp_path / "data" / "processed"
    processed.mkdir(parents=True)
    pd.DataFrame([
        {
    ...
    pd.DataFrame([
        {
            ...
            "reason": "template row still awaiting source verification",
        }
    ]).to_csv(overrides / "financial_facts.csv", index=False)
    ...
    assert unified.loc[0, "best_source_name"] == "smartlab"
    assert unified.loc[0, "manual_override_value"] is None
    assert unified.loc[0, "best_value"] == 100.0
